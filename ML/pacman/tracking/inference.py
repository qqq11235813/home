...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #print ("legalaction",self.legalPositions)
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                allPossible[p] = emissionModel[trueDistance]*self.beliefs[p] if noisyDistance!=None else 0.0
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        '''
        self.allParticles = []
        numLocation = len(self.legalPositions)

        for i in range(self.numParticles):
            distributeFlag = np.random.rand()*numLocation
            if distributeFlag!=numLocation:
                self.allParticles.append(self.legalPositions[int(distributeFlag)])
            else:
                self.allParticles.append(self.legalPositions[int(numLocation)])
        '''
        #@@######################################################################
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        for particleLocation in self.allParticles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particleLocation))
            for newPos in newPosDist.keys():
                allPossible[newPos] += newPosDist[newPos]

        allPossible.normalize()

        for i in range(self.numParticles):
            self.allParticles[i] = util.sample(allPossible)
    ...
    def getBeliefDistribution(self):
        ...
        for ele in self.allParticles:
            beliefDistribution[ele]+=1  
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        allGhostPosition = list(itertools.product(self.legalPositions,repeat = self.numGhosts))
        random.shuffle(allGhostPosition)

        count, self.allParticles = 0, []
        while count < self.numParticles:
            for position in allGhostPosition:
                if count < self.numParticles:
                    self.allParticles.append(position)
                    count += 1
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.allParticles:
        ...
        self.allParticles = newParticles
